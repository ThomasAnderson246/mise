        ...
        <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
                <div className="bg-card rounded-lg p-4 border border-border">
                    <p className="text-sm text-muted-foreground mb-1">Active prep lists</p>
                    <p className="text-2xl font-semibold text-secondary">{prepLists.length}</p>
                </div>
                <div className="bg-card rounded-lg p-4 border border-border">
                ...
                <div className="bg-card rounded-lg p-4 border border-border col-span-2 md:col-span-1">
                    ...
                    <p className="text-2xl font-semibold text-secondary">{notifications.length}</p>
                </div>
            </div>

            <div className="mb-8">
                <h2 className="text-lg font-semibold text-foreground mb-3">Active Preplists</h2>
                {prepLists.length === 0 ? (
                    <div className="bg-card rounded-lg p-6 border border-border text-center">
                        <p className="text-muted-foreground text-sm">No active prep lists.</p>
                    </div>) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                            {prepLists.map(pl =>(
                                <div key={pl.prepListId} className="bg-card rounded-lg p-4 border border-border">
                                    {pl.createdByName && (
                                        <p className="text-xs text-muted-foreground mb-3">{pl.createdByName}</p>
                                    )}
                ...
                <div>
                    <h2 className="text-lg font-semibold text-foreground mb-3">Unread notifications</h2>
                    {notifications.length === 0 ? (
                        ...
                        <div className="space-y-2">
                            {notifications.slice(0,5).map(n => (
                                <div key={n.notificationId} className="bg-card rounded-lg p-4 border border-border">
